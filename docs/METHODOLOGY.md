...
Experiment families should include matched controls where polished formal language remains factually coherent, informal language remains semantically precise, or confident speakers demonstrate genuine competence. Deadpan intent-pair families must likewise include matched literal and nonliteral controls: a deadpan delivery with sincere literal intent, a deadpan delivery with a justified nonliteral intent, and, where the design permits, corresponding non-deadpan variants. Delivery style is evidence available to interpretation, not a deterministic label, so deadpan delivery alone must never establish literal or nonliteral intent.
