...
if text.count(helper_marker) != 1:
    raise SystemExit("could not uniquely locate _visible_text")
...
text = text.replace(
    helper_marker,
    "\n" + helper + "def _visible_text(markdown: str) -> str:\n",
    1,
)

old_visible_head = '''def _visible_text(markdown: str) -> str:
...
'''
new_visible_head = '''def _visible_text(markdown: str) -> str:
    """Return browser-visible text without hidden HTML or link metadata."""
...
'''
if text.count(old_visible_head) != 1:
    raise SystemExit("could not uniquely patch _visible_text guard")
text = text.replace(old_visible_head, new_visible_head, 1)
...
new_rendered = '''    end, _ = _visible_markdown_heading_span(structure, WORKSTREAM_END_HEADING)
    ...
    return visible_structure[start:end]
'''
if text.count(old_rendered) != 1:
    raise SystemExit("could not uniquely patch policing workstream renderer")
text = text.replace(old_rendered, new_rendered, 1)
...
import_marker = "import runpy\n"
if text.count(import_marker) != 1:
    raise SystemExit("could not uniquely locate runpy import")
text = text.replace(import_marker, "import runpy\nimport re\n", 1)

function_start = text.index("def _visible_phase2_notes(changelog: str) -> str:")
...
'''
text = text[:function_start] + new_function + text[function_end + 1 :]
...
'''
phase2.write_text(text.rstrip() + regression + "\n", encoding="utf-8")
